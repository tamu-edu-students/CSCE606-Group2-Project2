...
Calorie Counter is a Ruby on Rails web application that allows users to log meals, calculate personalized macro targets, and stay accountable across devices.
It supports Google OAuth sign-in, onboarding surveys that tailor goals, AI-powered photo analysis for food entries, and an accessible dashboard that works even when JavaScript is disabled.
...
Slack Group (to track Scrum Events) - #project2-checkins (link shared in the course workspace)
...
You can execute git remote to show a list of all remote Git repos associated with your app, among which heroku should now appear. You can execute git remote show heroku to verify that pushing to the heroku "repo" will deploy to a URL on Heroku whose name matches the name you picked for your app.
...
Every time a pull request is merged into main, the app automatically deploys to Heroku through the GitHub Actions workflow setup in .github/workflows/ci.yml
You can verify deployment progress under the Actions tab in your repository.
The deployment starts only after all the checks/tests pass.
...
check last few lines of error log messages from Heroku	heroku logs --tail
reset the local database	rails db:drop db:create db:migrate

Project Diagrams
Database Schema Design (ER)
Database ERD (Diagrams/DBSchema.drawio.png)
...
Click Log a meal from the dashboard.
Provide a Food name and either:
Enter calories, protein, fats, and carbs manually, or
Upload a photo so the AI service can suggest values.
Click Create to save. Your new log appears grouped by date on the dashboard.
To edit, open a food entry, adjust values (or upload a new photo) and click Update.

Tip: Logs are sorted by most recent by default. Use the sort picker to change between date and macro-based ordering.

AI Photo Analysis
To analyze a meal photo:
Open a log form and attach an image.
Leave macro fields blank to trigger OpenAI Vision.
Submit the form. Suggested macros appear after processing and may be edited before saving.
If the AI call fails or the key is missing, the form displays a friendly error and keeps any manual values.

Managing Your Account
Update Goals
Go to Profile → Update goals.
Adjust weight, activity level, or goal (lose/maintain/gain) and submit. The system recalculates macros instantly.

Delete Account
From the same menu, choose Delete Account → Confirm to permanently remove your account and associated logs.

We designed Calorie Counter to respect user control — all personal data is scrubbed during deletion, and AI suggestions never persist without user confirmation.
...
Sample Logins (for Omniauth test mode)
...
Remember: these accounts exist only in local/test environments and are not persisted unless you save them.
...
Context
Accessibility requirements and grading rubric mandate full functionality without JavaScript, yet we still want responsive UX for modern browsers.
...
Date: 09-24-2024 Status: Resolved
...
Impact
User Experience:
All users saw "Authentication failed" after Google redirected back.
System Integrity:
No data loss, but sessions were never established so dashboards were inaccessible.
Business Impact:
High — blocked every login attempt in production.
...
Impact
User Experience:
Food log forms appeared to hang, and users sometimes resubmitted creating duplicates.
System Integrity:
Background threads piled up, increasing memory usage when timeouts stacked.
Business Impact:
Medium — manual entry still worked, but the flagship AI feature was unreliable.
...
bundle install fails	Ruby version mismatch	Run rbenv local 3.4.5 && bundle install
rails server won’t start	Missing GOOGLE_CLIENT_ID/SECRET	Set env vars or enable OmniAuth test mode
RSpec tests fail	Database not prepared	Run RAILS_ENV=test rails db:prepare
...
“ActiveRecord::PendingMigrationError” when starting server	Local migrations haven’t been applied	Run rails db:migrate
...
“Fuel smart decisions with every log.”
